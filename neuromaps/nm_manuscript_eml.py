# -*- coding: utf-8 -*-
"""NeuroMap manuscript: приращение d = dt * g(u,p), где g задаётся EML-деревьями (по одному на координату)."""
...
from .eml_tree import TrainableEMLReductionTree
from .nm_manuscript import HistoryCallback


class NeuroMapManuscriptEML(pl.LightningModule):
    """
    Как NeuroMapManuscript, но вместо MLP — набор TrainableEMLReductionTree
    (одно дерево на каждую координату приращения).
    """

    def __init__(self, n_var, n_param, n_leaves=8, dt=0.01, lr=1e-3):
        super().__init__()
        self.save_hyperparameters("n_var", "n_param", "n_leaves", "dt", "lr")
        ...
        self.n_terminals = 1 + n_var + n_param
        self.eml_trees = nn.ModuleList(
            [TrainableEMLReductionTree(self.n_terminals, n_leaves=n_leaves) for _ in range(n_var)]
        )
        self.criterion = nn.L1Loss()

    def _terminals(self, u: torch.Tensor, p: torch.Tensor) -> torch.Tensor:
        batch = u.shape[0]
        ones = torch.ones(batch, 1, device=u.device, dtype=u.dtype)
        return torch.cat([ones, u, p], dim=1)

    def forward(self, u, p):
        t = self._terminals(u, p)
        parts = [self.eml_trees[i](t) for i in range(self.n_var)]
    ...
    def compute_d_and_jacobian(self, u, p):
        """d и якобиан dd/du (автоград по EML)."""
        self.eval()
        ...
        with torch.enable_grad():
            ...
            ones = torch.ones(1, 1, device=self.device, dtype=torch.float32)
            terminals = torch.cat([ones, u_t, p_t], dim=1)
            g = torch.cat([self.eml_trees[i](terminals) for i in range(self.n_var)], dim=1)
            d_vec = (self.dt * g).squeeze(0)
            J_rows = []
    ...
    def load(cls, path, device=None):
        ...
        if "_format" in checkpoint and checkpoint["_format"] == "neuromap_eml_v1.0":
            hparams = checkpoint.get("hyper_parameters", {})
            required = ["n_var", "n_param", "n_leaves", "dt", "lr"]

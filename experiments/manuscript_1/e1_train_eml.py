"""
Обучение NeuroMapManuscriptEML на том же датасете, что и e1_train.py (manuscript_1).

Чекпоинты: experiments/manuscript_1/checkpoints_eml/
"""
import numpy as np
...
CHECKPOINT_DIR = Path("experiments/manuscript_1/checkpoints_eml")
...
if checkpoint_files:
    ...
    model = NeuroMapManuscriptEML.load(str(latest_ckpt))
else:
    print("Создаём новую EML-модель…")
    model = NeuroMapManuscriptEML(
        n_var=2,
        n_param=2,
        n_leaves=8,
        dt=DT,

...
def main() -> None:
    ...
    if not model_path.is_file():
        raise FileNotFoundError("Run 02_train_model3_subnets.py first.")

    model = NeuroMapManuscriptSubnets.load(str(model_path), device="cpu")

    x_grid = np.linspace(-10.0, 10.0, 100)
    y_grid = np.linspace(-135.0, 135.0, 100)
    ...
    np.savez(RESULTS_DIR / "e1_amplitude_basin.npz", x_grid=x_grid, y_grid=y_grid, Z_ode=Z_ode, Z_nm=Z_nm)

    alpha_grid = np.linspace(-3.0, 1.0, 20)
    beta_grid = np.linspace(0.02, 0.1, 20)
    params = [alpha_grid, beta_grid]
    x_init_grid = np.linspace(-10.0, 10.0, 10)
    y_init_grid = np.linspace(-10.0, 10.0, 10)
    ...
    np.savez(RESULTS_DIR / "e1_ode_fixed_point_probability.npz", alpha_grid=alpha_grid, beta_grid=beta_grid, P=P_ode)
    ...
    np.savez(RESULTS_DIR / "e1_neuromap_fixed_point_probability.npz", alpha_grid=alpha_grid, beta_grid=beta_grid, P=P_nm)

    u1_axis = np.linspace(-15.0, 15.0, 12)
    u2_axis = np.linspace(-35.0, 35.0, 12)
    ...
    np.savez(RESULTS_DIR / "e1_neuromap_scan_residual_1em7.npz", lam_grid=xg, beta_grid=yg, dist_Z=dist_Z, log_rho_Z=log_Z, xg=xg, yg=yg)
